...
/* ── Inline style helper to avoid page-break inside a block ──────────── */
const noBreak: React.CSSProperties = { pageBreakInside: "avoid", breakInside: "avoid" };
...
export const PrintableInterviewReport = React.forwardRef<HTMLDivElement, Props>(({ detail, config }, ref) => {
  ...
  const pc = config || { overview: true, candidateFeedback: true, interviewerQuality: true, transcript: true };
  ...
  return (
    ...
    >
      {/* ═══════════ HEADER — always shown ═══════════ */}
      <div style={{ ...noBreak, paddingBottom: "20px", borderBottom: "3px solid #4f46e5", marginBottom: "30px" }}>
      ...
      {/* ═══════════ PAGE 1 — OVERVIEW & RECRUITER SUMMARY ═══════════ */}
      {pc.overview && (
        <>
          {/* Candidate Profile Card */}
          <div style={{ ...noBreak, backgroundColor: "#f8fafc", padding: "20px", borderRadius: "8px", marginBottom: "30px", border: "1px solid #e2e8f0" }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: "20px" }}>
              ...
              <div style={{ flex: "1 1 45%" }}>
                <p style={{ margin: "0 0 5px 0", fontSize: "13px", color: "#64748b", textTransform: "uppercase", fontWeight: "bold" }}>Interview Date</p>
                <p style={{ margin: 0, fontSize: "16px", fontWeight: "600", color: "#0f172a" }}>{detail.created_at?.slice(0, 10) || "—"}</p>
              </div>
              {(detail.tab_switch_count ?? 0) > 0 && (
                <div style={{ flex: "1 1 45%" }}>
                  <p style={{ margin: "0 0 5px 0", fontSize: "13px", color: "#dc2626", textTransform: "uppercase", fontWeight: "bold" }}>⚠ Tab Switches</p>
                  <p style={{ margin: 0, fontSize: "16px", fontWeight: "600", color: "#dc2626" }}>{detail.tab_switch_count} detected</p>
                </div>
              )}
            </div>
          </div>

          {/* Score & Verdict — role_fit_score + overall_score */}
          <div style={{ ...noBreak, display: "flex", gap: "15px", marginBottom: "35px" }}>
            <div style={{ backgroundColor: "#eff6ff", border: "1px solid #bfdbfe", padding: "20px", borderRadius: "12px", flex: 1, textAlign: "center" }}>
            ...
            {ir.role_fit_score != null && (
              <div style={{ backgroundColor: "#f5f3ff", border: "1px solid #ddd6fe", padding: "20px", borderRadius: "12px", flex: 1, textAlign: "center" }}>
                <div style={{ fontSize: "13px", color: "#6d28d9", textTransform: "uppercase", fontWeight: "800", letterSpacing: "1px" }}>Role Fit Score</div>
                <div style={{ fontSize: "48px", fontWeight: "900", color: "#4c1d95", marginTop: "5px" }}>{ir.role_fit_score}</div>
                <div style={{ fontSize: "12px", color: "#7c3aed", fontWeight: "600" }}>out of 100</div>
              </div>
            )}
            <div style={{ backgroundColor: detail.verdict === 'NO HIRE' ? '#fef2f2' : detail.verdict === 'HOLD' ? '#fffbeb' : '#f0fdf4', border: "1px solid", borderColor: detail.verdict === 'NO HIRE' ? '#fecaca' : detail.verdict === 'HOLD' ? '#fde68a' : '#bbf7d0', padding: "20px", borderRadius: "12px", flex: 1, textAlign: "center" }}>
          ...
          {/* Executive Summary */}
          <div style={{ ...noBreak, marginBottom: "35px" }}>
          ...
          {/* Competency Evaluation — Technical, Behavioral, Communication, Overall */}
          {ir.competency_summary && (
            <div style={{ ...noBreak, marginBottom: "35px" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", gap: "12px" }}>
                {[
                  { label: "Technical", value: ir.competency_summary.technical_avg, color: "#2563eb" },
                  { label: "Behavioral", value: ir.competency_summary.behavioral_avg, color: "#8b5cf6" },
                  { label: "Communication", value: ir.competency_summary.communication_avg, color: "#10b981" },
                  { label: "Overall", value: ir.competency_summary.overall_avg, color: "#f59e0b" },
                ].map((c) => (
                  <div key={c.label} style={{ flex: 1, backgroundColor: "#ffffff", padding: "15px", borderRadius: "8px", border: "1px solid #e2e8f0", textAlign: "center" }}>
                    <div style={{ fontSize: "12px", color: "#64748b", textTransform: "uppercase", fontWeight: "700", marginBottom: "8px" }}>{c.label}</div>
                    <div style={{ fontSize: "24px", fontWeight: "900", color: c.color }}>{c.value ?? "N/A"}</div>
                  </div>
                ))}
              </div>
            </div>
          ...
          {/* Integrity Warning (Tab Switches) */}
          {(detail.tab_switch_count ?? 0) > 0 && (
            <div style={{ ...noBreak, marginBottom: "35px", backgroundColor: "#fef2f2", padding: "15px 20px", borderRadius: "8px", border: "1px solid #fecaca" }}>
              <h3 style={{ fontSize: "14px", fontWeight: "bold", color: "#dc2626", margin: "0 0 8px 0" }}>⚠ Integrity Warning</h3>
              <p style={{ margin: 0, fontSize: "13px", color: "#991b1b", lineHeight: "1.5" }}>
                The candidate switched tabs or minimized the browser <strong>{detail.tab_switch_count}</strong> times during the interview. This behavior may indicate unauthorized assistance.
              </p>
            </div>
          )}

          {/* Key Strengths & Concerns */}
          <div style={{ ...noBreak, display: "flex", gap: "25px", marginBottom: "35px" }}>
            <div style={{ flex: 1 }}>
          ...
          {/* Recruiter Insights — Culture Fit, Salary Positioning, Next Steps */}
          <div style={{ ...noBreak, backgroundColor: "#f8fafc", padding: "20px", borderRadius: "8px", border: "1px solid #e2e8f0" }}>
      ...
      {/* ═══════════ PAGE 2 — CANDIDATE PERFORMANCE REVIEW ═══════════ */}
      {pc.candidateFeedback && (
        <div style={{ pageBreakBefore: pc.overview ? "always" : "auto", paddingTop: "10px" }}>
          ...
          {/* Grade + Performance Score + Overall Assessment */}
          <div style={{ ...noBreak, display: "flex", gap: "15px", marginBottom: "25px", alignItems: "center" }}>
            <div style={{ backgroundColor: "#e0e7ff", padding: "15px", borderRadius: "10px", border: "1px solid #c7d2fe", textAlign: "center", width: "100px" }}>
            ...
            {cr.performance_score != null && (
              <div style={{ backgroundColor: "#f0fdf4", padding: "15px", borderRadius: "10px", border: "1px solid #bbf7d0", textAlign: "center", width: "100px" }}>
                <div style={{ fontSize: "12px", color: "#166534", fontWeight: "bold", textTransform: "uppercase" }}>Score</div>
                <div style={{ fontSize: "32px", fontWeight: "900", color: "#14532d", marginTop: "4px" }}>{cr.performance_score}</div>
              </div>
            )}
            <div style={{ flex: 1 }}>
          ...
          {/* Integrity Warning in Candidate Section too */}
          {(detail.tab_switch_count ?? 0) > 0 && (
            <div style={{ ...noBreak, marginBottom: "25px", backgroundColor: "#fef2f2", padding: "12px 15px", borderRadius: "8px", border: "1px solid #fecaca" }}>
              <p style={{ margin: 0, fontSize: "13px", color: "#991b1b" }}>
                <strong>⚠ Integrity Note:</strong> {detail.tab_switch_count} tab switch(es) detected during the interview.
              </p>
            </div>
          )}

          {/* Strengths & Improvements */}
          <div style={{ ...noBreak, display: "flex", gap: "25px", marginBottom: "30px" }}>
            <div style={{ flex: 1, backgroundColor: "#f0fdf4", padding: "20px", borderRadius: "8px", border: "1px solid #bbf7d0" }}>
              <h3 style={{ fontSize: "15px", fontWeight: "800", color: "#166534", marginBottom: "15px" }}>
                ✓ What Went Well
              </h3>
            ...
            <div style={{ flex: 1, backgroundColor: "#fef2f2", padding: "20px", borderRadius: "8px", border: "1px solid #fecaca" }}>
              <h3 style={{ fontSize: "15px", fontWeight: "800", color: "#991b1b", marginBottom: "15px" }}>
          ...
          {/* Alternative Roles & Skill Development */}
          <div style={{ ...noBreak, display: "flex", gap: "25px", marginBottom: "30px" }}>
          ...
          {cr.interview_tips && cr.interview_tips.length > 0 && (
            <div style={{ ...noBreak, backgroundColor: "#fffbeb", padding: "15px 20px", borderRadius: "8px", border: "1px dashed #fcd34d" }}>
      ...
      {/* ═══════════ PAGE 3 — INTERVIEWER QUALITY AUDIT ═══════════ */}
      {pc.interviewerQuality && (
        <div style={{ pageBreakBefore: (pc.overview || pc.candidateFeedback) ? "always" : "auto", paddingTop: "10px" }}>
          ...
          {/* Scores Row — question_quality, coverage, interviewer_rating, question_diversity */}
          <div style={{ ...noBreak, display: "flex", gap: "12px", marginBottom: "25px", flexWrap: "wrap" }}>
            {[
              { label: "Question Quality", value: iq.question_quality_score },
              { label: "JD Coverage", value: iq.competency_coverage_percent ? `${iq.competency_coverage_percent}%` : undefined },
              { label: "Interviewer Rating", value: iq.interviewer_rating },
              { label: "Question Diversity", value: iq.question_diversity },
            ].map((m) => (
              <div key={m.label} style={{ flex: "1 1 22%", backgroundColor: "#f0fdfa", padding: "15px", borderRadius: "8px", border: "1px solid #ccfbf1", textAlign: "center" }}>
                <div style={{ fontSize: "11px", color: "#0f766e", textTransform: "uppercase", fontWeight: "bold" }}>{m.label}</div>
                <div style={{ fontSize: "24px", fontWeight: "900", color: "#115e59", marginTop: "4px" }}>{m.value ?? "—"}</div>
              </div>
            ))}
          </div>

          {/* Interviewer Feedback */}
          <div style={{ ...noBreak, backgroundColor: "#f8fafc", padding: "20px", borderRadius: "8px", border: "1px solid #e2e8f0", marginBottom: "25px" }}>
          ...
          <div style={{ ...noBreak, display: "flex", gap: "25px", marginBottom: "25px" }}>
            <div style={{ flex: 1 }}>
          ...
          {(iq.coverage_gaps?.length > 0 || iq.bias_indicators?.length > 0) && (
            <div style={{ ...noBreak, display: "flex", gap: "25px" }}>
              {iq.coverage_gaps && iq.coverage_gaps.length > 0 && (
                <div style={{ flex: 1 }}>
                  ...
                  <ul style={{ margin: 0, paddingLeft: "18px", fontSize: "12px", color: "#92400e" }}>
                    {iq.coverage_gaps.map((gap: string, i: number) => <li key={i} style={{ marginBottom: "4px" }}>{gap}</li>)}
                  </ul>
                </div>
              )}
              {iq.bias_indicators && iq.bias_indicators.length > 0 && (
                <div style={{ flex: 1 }}>
                  ...
                  <ul style={{ margin: 0, paddingLeft: "18px", fontSize: "12px", color: "#991b1b" }}>
                    {iq.bias_indicators.map((bias: string, i: number) => <li key={i} style={{ marginBottom: "4px" }}>{bias}</li>)}
                  </ul>
                </div>
              )}
            </div>
      ...
      {pc.transcript && pt && pt.length > 0 && (
        <div style={{ pageBreakBefore: (pc.overview || pc.candidateFeedback || pc.interviewerQuality) ? "always" : "auto", paddingTop: "10px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "15px" }}>
            {pt.map((qa: any, index: number) => (
              <div key={index} style={{ ...noBreak, backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", borderRadius: "8px", overflow: "hidden" }}>
